...
def read_frame_to_torch(path):
    ...
    input_image = input_image.unsqueeze(0) / 255  # Normalize to [0, 1]

    # Resize the tensor to [1, 3, 1024, 1920]
    input_image = torch.nn.functional.interpolate(
        input_image, size=(1024, 1920), mode='bilinear', align_corners=False
    )
    return input_image
...
def encode_one(args_dict, device):
    ...
    dataset_full_path = os.path.join("/w/331/lgupta/DCVC/DCVC/media/data/vcd/", sub_dir_name)
    # dataset_full_path = os.path.join(args_dict['dataset_path'], sub_dir_name)
    print(f"Checking dataset path: {dataset_full_path}")
    pngs = os.listdir(dataset_full_path)  # This is where the error occurs
    if 'im1.png' in pngs:
        padding = 1
    ...
    with torch.no_grad():
        for frame_idx in range(frame_num):
            ori_frame = read_frame_to_torch(
                os.path.join("/w/331/lgupta/DCVC/DCVC/media/data/vcd",
                             sub_dir_name,
                             f"im{str(frame_idx+1).zfill(padding)}.png"))
            print(f"Input frame shape after resizing: {ori_frame.shape}")
